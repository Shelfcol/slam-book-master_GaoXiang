...
public:
    ...
    virtual void computeError() override {
		...
		//源代码中的实现
		//李群相机位姿v1  存储顶点信息，比如二元边的话，_vertices[] 的大小为2
...
int main(int argc, char **argv) {
    ...
    while (!fin.eof()) {
        ...
        //获取SE(3)下的相机位姿，这里有三张图片，所以有三个相机位姿
    ...
    vector<float *> color;
    fin.open(points_file);//points.txt中存储的是3D点集合以及该点周围 4x4 窗口的灰度值，记做 I(p)i，格式为：x, y, z, 灰度1，灰度2...，灰度16
    while (!fin.eof()) {
        double xyz[3] = {0};
	...
	//有三张照片相机位姿，3D点集合，该点周围 4x4 窗口的灰度值
