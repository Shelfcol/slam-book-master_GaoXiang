...
int main(int argc, char** argv) {
  ...
  noiseModel::Diagonal::shared_ptr priorModel = noiseModel::Diagonal::Sigmas(Vector3(1.0, 1.0, 0.1));//设定一个测量的噪声模型
  graph.add(PriorFactor<Pose2>(Symbol('x', 1), Pose2(0, 0, 0), priorModel));//设定一个初始分布的factor，必须固定一个初始的分布值，否则整张图可以进行平移，都能满足条件
  ...
  // Add odometry factors 运动模型
  // Create odometry (Between) factors between consecutive poses
  // robot makes 90 deg right turns at x3 - x5
  graph.add(BetweenFactor<Pose2>(Symbol('x', 1), Symbol('x', 2), Pose2(5, 0, 0), odomModel));//between设置两个factor之间相差多少，这里假设x方向走5米
  ...
  graph.add(BetweenFactor<Pose2>(Symbol('x', 5), Symbol('x', 2), Pose2(5, 0, -M_PI_2), loopModel));//加一个闭环
  ...
  Values initials;//Values类是一个container
  initials.insert(Symbol('x', 1), Pose2(0.2, -0.3, 0.2));//pose2是在真实值上加了噪音，实际就是带噪声的测量值。或者用开环的里程计读书，或者用GPS获取
